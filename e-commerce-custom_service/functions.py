...
def login_account(context_variables:dict) -> Union[str, SwarmResult]:
    ...
    if login_in_status:
        ...
        return SwarmResult(agent="order_management_agent", values=f"User is successfully logged in. {user_preferrance_str}", context_variables=context_variables)
    else:
...
def check_return_eligibility(order_number:str, context_variables: dict) -> str:
    ...
    if orders[order_number]['status'] != "delivered":
        return "The order is not delivered yet, please wait until the order is delivered"
...
def initiate_return_process(order_number:str , context_variables: dict) -> Union[str, SwarmResult]:
    orders = context_variables['user_info']['orders']
    if not check_return_eligibility(order_number, context_variables) == "The order is eligible for return":
...
# for tracking agent
def verify_order_number(order_number:str, context_variables:dict) -> Union[str, SwarmResult]:
    ...
    return SwarmResult(values="The order number is valid.", context_variables=context_variables)    

# for tracking agent
def verify_user_information(email:str=None, phone_number_last_4_digit:str=None, context_variables:dict=None) -> str:
    ...
    if phone_number_last_4_digit:
        if phone_number_last_4_digit.strip() == context_variables["order_info"]["phone_number"][-4:]:
            return order_str
